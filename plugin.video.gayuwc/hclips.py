...
def Categories(url):
    ...
    for catpage, name, img, in match:
    	if name.lower().find('gay') != -1: utils.addDir(name, catpage, 381, img, '')
    	else: olist.append((name, catpage, 381, img, ''))
    try:
		for name, catpage, catnum, img, ofield in olist:
			utils.addDir(name, catpage, 381, img, '')
    except Exception, e:
    	raise e
    	print str(e)
        #if name.startswith('Gay'): utils.addDir(name, catpage, 381, img, '')
